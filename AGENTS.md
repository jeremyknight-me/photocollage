...
#### Formatting
- Follow `.editorconfig` in `src/`.
- Use `nameof` instead of string literals when referring to member names.

#### Nullable Reference Types
- Declare variables non-nullable; check for `null` at entry points.
- Always use `is null` or `is not null` instead of `== null` / `!= null`.
- Trust null annotations; don't add null checks the type system says are unnecessary.
